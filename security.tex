...
\begin{proof}
  ...
  Let $\hatmT$ be the transactions to be considered by $p_i$'s snaphot: $\hatmL=\barmU\applytx\hatmT$
  where $\barmU$ is the snaphot prior to $p_i$'s. Since $p_i$ issues
  $(\hpRT,\tx)$ after each snapshot, we have that, either,
  \begin{itemize}
      \item $\tx\in\hatmT$, in which case $\tx \in \bigcap_{i\in[n]} \Tbar_i$, or,
      \item $\tx\notin\hatmT$, in which case $\hatmL\applytx\tx=\bot$ ($\tx$ is still in the wait queue of $(\hpRT,\tx)$. Since $\hatmL=\Uinit\applytx\cap
  \end{itemize}
