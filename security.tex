...
\item $\Snapbar_i$: snapshots $(s,U)$ that party
  $\party_i$ performs \emph{while uncorrupted}: output $(\hpSnap,(s,U))$;
...
\item \propName{Consistency (Head)}: In presence of an active adversary, the
  ...
  \item \propName{Liveness (Head)}:
    ...
    while $\argmax_s(\Snapbar_i)\applytx\tx\not=\bot$.
...
Note that the original version of the coordinated head satisfied stronger versions of liveness that was not explicitly stated---but still important, for instance:

\begin{itemize}
  \item \propName{Optimistic Liveness (Head)}:
    Consider any protocol execution in presence of a network adversary wherein
    the head does not get closed for a sufficiently long period of time, and consider
    an honest party $p_i$ who enters transaction $\tx$ by executing $(\hpNew,\tx)$
