...
\todo{maybe change terminology (use L1 and L2)}
...
optimal\todo{The word `optimal' can be triggering for some---maybe just `increasing throughput', unless can point to a theoretical result on optimality} way. At the same time, the head protocol provides the same capabilities
as the mainchain by reusing the same ledger model and transaction formats --- it's \todo{What is `it' in this case?}
isomorphic.
...
not allowed to close the head) and for multisignature-based \todo{Would be good to describe what is multisig} event confirmation
in the head.
...
state $\stInitial$, each head member can attach a \mtxCom{} transaction\todo{Is there a reason why `commit txs' has a different font than others}, which
...
from $\stInitial$ to $\stFinal$. \todo{Can some head members not commit a tx but still participate?}

\todo{General comment: It is not very clear here, which txs are offchain which onchain.}
...
concurrency\todo{what is meant by full concurrency here?} off-chain, while each party maintains their view of the local UTxO
...
$\Uset_1,\Uset_2,\ldots$ \todo{$\Uset_{1}$ overlaps with committed utxo $U_{1}$
  of party $1$} are created with monotonically increasing snapshot numbers.

For this, a snapshot leader \todo{Who gets to be leader?} requests his view of a new confirmed state to be
...
\todo{General comment: ensure consistency, i.e. `offchain' or `off-chain'}
\subsection{Differences}
\todo{More details on what was in orig paper?}
