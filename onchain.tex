...
instance is represented by the unique\footnote{The uniqueness of $\cid$ is guaranteed because the EUTxO ledger ensures that $\txOutRef_{\mathsf{seed}}$ cannot be spent twice in the same chain} currency identifier $\cid$ created by minting tokens using
...
\begin{itemize}
  \item A single \emph{State Thread (ST)} token marks the output, which carries the state
        of the protocol on-chain. The token name is the well known string
  ...
  \item One \emph{Participation Token (PT)} per participant
        $i \in \{1 \dots \nop \}$ to be used for authenticating further transactions and to ensure every participant can commit. The token name is the participant's
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/SM-init-commit.pdf}
...
\begin{enumerate}
  ...
  \item No minting or burning $\txMint = \varnothing$
\end{enumerate}https://www.overleaf.com/project/6389ba5edbcf7a51fda1328f
...
\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth]{figures/SM-fanout.pdf}
