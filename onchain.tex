...
\begin{itemize}
  ...
  \item One \emph{Participation Token (PT)} per participant
        ...
        $\pt_{i} = (\cid \rightarrow \keyHash_{i} \rightarrow 1)$.\todo{$\pt = (\cid \rightarrow \keyHash_{i})$}\todo{refer to protocol setup}
\end{itemize}
...
\begin{itemize}
  ...
  \item Zero or one input with reference $\txOutRef_{commit}$ spending output
        $o_{committed} = (\val_{committed}, \cdot , \cdot)$ \todo{very restrictive, allow more?}
