...
ledger's validation of the transaction. The protocol defines three different
scripts: \todo{add minting script}
\begin{itemize}
...
\item $\mathsf{PT}_i,\ i\in \{1 \dots \nop \}$: One \emph{Participation Token} per participant, where the token name is the participant's verification key hash $k_i^{\#}$. This will be used to authenticate participants in protocol transactions.
\end{itemize}
