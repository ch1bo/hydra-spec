...
\begin{itemize}
  \item One input $i_{\mathsf{initial}} = (\txOutRef_{\mathsf{initial}}, \redeemer_{\mathsf{initial}})$
        spending $o_{\mathsf{initial}} = (\val_{\mathsf{initial}}, \nuInitial, \datum_{\mathsf{initial}})$
  ...
  \item One output $o_{\mathsf{commit}} = (\val_{\mathsf{commit}}, \nuCommit, \delta_{\mathsf{commit}})$
\end{itemize}

\noindent The $\nuInitial$ validator with $\datum_{\mathsf{initial}} = \cid$ and $\redeemer_{\mathsf{initial}} = \txOutRef_{\mathsf{committed}}$ ensures that:
\begin{menumerate}
  \item The committed value is in the output $\val_{\mathsf{commit}} = \val_{\mathsf{initial}} \cup \val_{\mathsf{committed}}$
  \item The currency id and committed output are recorded in the output datum
  $\delta_{\mathsf{commit}} = (\cid, U_{i})$ where
  $U_{i} = (\txOutRef_{\mathsf{committed}},\bytes(o_{\mathsf{committed}}))$
  \item Transaction is signed by a participant $\exists \{\cid \mapsto \keyHash_{i} \mapsto 1\} \in \val_{\mathsf{commit}} \Rightarrow \keyHash_{i} \in \txKeys$
  \item No minting or burning  $\txMint = \varnothing$\todo{restrict only $\cid \notin \txMint$?}
\end{menumerate}
...
\begin{menumerate}
  ...
  which are sorted by their associated $\txOutRef$\footnote{Sorting is required
    ...
    \todo{need to rewrite this using projections $\downarrow1$ syntax}
  \[
    \combine(\underline{U}) = \hash(\mathsf{concat}^{\downarrow2}(\sortByOutRef^{\downarrow1}(\underline{U})))
  \]
...
\begin{menumerate}
  ...
  $\{\st \mapsto 1\} \in \val'$, where $(\cid,\cdot) = \delta_{\mathsf{commit}}$ is given
