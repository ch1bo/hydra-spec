...
\begin{menumerate}
  ...
  $\delta_{com} = (\cid, U_{i})$ where
  $U_{i} = (\txOutRef_{commit},\bytes(o_{commit}))$
  \item Transaction is signed by a participant $\exists (\cid \rightarrow \keyHash_{i} \rightarrow 1) \in \val_{com} \Rightarrow \keyHash_{i} \in \txKeys$
...
\begin{menumerate}
  ...
  \item Transaction is signed by a participant $\exists (\cid \rightarrow \keyHash_{i} \rightarrow 1) \in \val_{com} \Rightarrow \keyHash_{i} \in \txKeys$\todo{also check $\keyHash_{i} \in \hppuv$?}
...
\begin{menumerate}
  ...
  $(\cid,.) = \delta_{com}$ is given by the datum of the commit output
  $o_{com}$.
\end{menumerate}
...
\begin{itemize}
  ...
  \item $\forall i \in \{1 \dots \nop\}$ inputs either
    \begin{itemize}
      ...
      \item spending from already committed output $(\val_{com_i}, \nuCommit, (\cid, U_{i}))$ with $\pt_{i} \in \val_{com_{i}}$,\todo{detail datums below?}
    \end{itemize}
...
\begin{menumerate}
  \item All UTxOs committed into the head are directly reimbursed\todo{check this}
  \[
    \underline{U}_{committed} = \sum_{i=i}^{\nop} U_{i} \neq \bot \Rightarrow U_{i}
  \]
  \[
    \forall j \in \{1\dots|\underline{U}_{committed}|\}: \hash(\txOutputs[j]) = \underline{U}_{committed}[j][2]
  \]\todo{notation: access on tuples}
...
\begin{menumerate}
  \item The ST is getting burned
  $(\cid \rightarrow \texttt{HydraHeadV1} \rightarrow -1) \subseteq \txMint$,
  where $\cid$ is given by the datum of the spent commit output $(\cid,.) = \delta_{com}$.
\end{menumerate}
...
\begin{enumerate}
  \item Signer is one of the participants: $\txKeys = \{\mathsf{k'}\}$, $k' \in \hppuv$ and
    $
    \exists (\cid \rightarrow \mathsf{PT}_i \rightarrow 1) \in \val, \hash(\mathsf{k'}) = \mathsf{PT}_i,
    $
