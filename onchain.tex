...
\begin{menumerate}
	...
	\item \textcolor{red}{Currency id and committed output are recorded in the output datum}
...
\begin{itemize}
	...
	\item one output paying to $\nuHead$ with value $\val_{\mathsf{head}}'$ and
	      datum $\datumHead'$.
\end{itemize}
...
\begin{menumerate}
	...
	\item \textcolor{red}{All committed value captured and no additional funds ``enter'' or ``leave''}\\
...
\begin{samepage}
	\noindent The state-machine validator $\nuHead$ is spent with
	$\redeemerHead = (\mathsf{abort}, m)$, where $m$ is the number of to reimburse
	outputs, and checks:
	\begin{menumerate}
		...
		\] % XXX: What does this actually mean?
