...
\begin{enumerate}
	...
	\item client commands as received from the environment
	      ...
	      \end{itemize}
\end{enumerate}
\todo{define states and e.g. that $\hpNew$ not possible when closed? state diagram?}
...
\begin{itemize}
	...
	\item The lifecycle of a Hydra head on-chain does not cross (hard fork)
	      ...
	      accordingly.\todo{Treat this better in a dedicated section, esp. in presence of rollbacks}
\end{itemize}

\subsection{Notation}
\todo{missing:, apply tx, map access}
\begin{itemize}
	...
	\item $\Req~p$ means that boolean expression $p \in \tyBool$ must be satisfied
	      for the further execution of a routine, while discontinued on $\neg p$. \todo{A conservative actor could interpret this as a reason to close the head}
...
setup.\todo{Actually: $\hydraKeys$ (not aggregated) in the init tx} While the
...
pending transaction set $\hatmT$ gets pruned by the \todo{outdated}just requested transactions
$\mT_{res}$ and the
