...
\dparagraph{$\hpRS$.}\quad Upon receiving request $(\hpRS,s,T)$ from party $\party_j$,
...
transactions referred in $T$ \todo{resolve $T \rightarrow \mT$} are
resolvable\todo{also "output seen" for security}. Only then, $\party_i$
...
the initial UTxO set $\Uinit$, a hash\todo{order + hash like~\ref{sec:close-tx}}
