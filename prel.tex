...
\[
  \nu : (\del\txDataTy, \txDataTy, \txPendingTxTy)
  \to\txBoolTy,
\]
where: 
\begin{itemize}
    \item $\txDataTy$ is a universal data type,
\end{itemize}
 ($\delta$) and redeemers ($\rho$), $\txBoolTy = \{\bot, \top\}$, and $\txPendingTxTy$ is the validation context. We also
