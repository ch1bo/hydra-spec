...
by~\cite{eutxo-2}~and~\cite{eutxo}. Values $a$ are of a set $a \in \mathcal{A}$,
also indicated as being of some type $a : \mathcal{A}$, and complex values are
tuples drawn from a $\times$ product of multiple sets, e.g.
$(a,b) \in (\mathcal{A} \times \mathcal{B})$. An empty set is indicated by
$\emptyset$ and sets may be enumerated using $\{\}$. The $=$ operator means
...
${(a,b)}^{\downarrow1} = a$. Functions are morphisms mapping from one to another
set $x : \mathcal{A} \to f(x) : \mathcal{B}$, where function
...
\begin{itemize}
  ...
  \item $\hash : x \to \tyBytes$ denotes a cryptographic collision-resistant
...
\begin{itemize}
  ...
  \item $\msCSig \gets \msComb(\msParams,\msMsg,\msVKL,\msSigL)$ aggregates a
        list $\msSigL$ \todo{rather use $\underline{\sigma}$} of signatures into a single, aggregate
        signature~$\msCSig$.
