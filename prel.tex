...
\begin{definition}[Inputs]
	An input $i \in \txInputs$ is an output reference $\txOutRef \in \tyOutRef$
	and a corresponding redeemer $\rho \in \tyData$:
	\[
		i \in \txInputs = (\txOutRef : \tyOutRef \times \rho : \tyData)
	\]
	where $\tyOutRef = (\tyBytes \times \mathbb{N})$ references an output of a
