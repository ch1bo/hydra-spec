...
\todo{be specific!?}
\noindent A multisignature scheme is a tuple of algorithms:
\[
...
\begin{itemize}
  ...
  \item $\msCSig \gets \msComb(\msParams,\msMsg,\msVKL,\msSigL)$ aggregates a
        list $\msSigL$ of signatures into a single, aggregate
        signature~$\msCSig$.
...
\begin{definition}[Validation Context]
  ...
  \textbf{set} of inputs, $\txOutputs \in \tyOutputs$ is a \textbf{list} of
  outputs, $\txMint \in \tyValue$ is the minted (or burned) value,
